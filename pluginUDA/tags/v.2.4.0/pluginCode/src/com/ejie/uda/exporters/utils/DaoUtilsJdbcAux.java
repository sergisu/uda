...
public class DaoUtilsJdbcAux {
	...
	public List<String[]> camposQueryUpdate(POJOClass pojo, Configuration cfg){
		...
		try{
			...
			//Juntamos primero el result y luego la primaria, en ese orden porque el orden de los parametros importa
	...
	private String obtenerCadenaResultSet(POJOClass pojo, Property propiedad){
	...
	public String rowmapperPK(POJOClass pojo, Configuration cfg){
		
		String strResulSetPks = "";
		try{
			Iterator<Property> itPropiedades= pojo.getAllPropertiesIterator();
			PersistentClass clazz = (PersistentClass) pojo.getDecoratedObject();
			while (itPropiedades.hasNext()){
				...
				if (!c2h.isCollection(propiedad)){
					...
					if(clazz.getIdentifierProperty().isComposite() && propiedad.equals(clazz.getIdentifierProperty())){
						...
						while(primarias.hasNext()){
							...
							if (c2h.isManyToOne(prim) || c2h.isOneToMany(prim)){
								...
								String nombreSubclass =   ControllerUtils.findNameFromEntity(subclass.getEntityName());
								pojo.importType(pojo.getPackageName()+".model."+ nombreSubclass);
								if (strResulSetPks.equals("")){
									strResulSetPks=strResulSetPks+"new "+nombreSubclass+"(" ;
								}else{
									strResulSetPks=strResulSetPks+", new "+nombreSubclass+"(" ;
								}
								...
								if (c2j.isComponent(subclass.getIdentifier())){
									...
									while (primRelacionada.hasNext()){
										...
										if (primar.isComposite() || c2h.isOneToMany(primar) || c2h.isOneToOne(primar) || c2h.isManyToOne(primar) ){
											...
											if (strResulSetPks.equals("") || strResulSetPks.substring(strResulSetPks.length()-1,strResulSetPks.length()).equals("(")){
												strResulSetPks=strResulSetPks+"new "+ nombreSubClaseMap +"(";
											}else	{
												strResulSetPks=strResulSetPks+", new "+ nombreSubClaseMap +"(";
											}	
											...
											if(primar.isComposite()){
												...
												while (listaFieldPrimar.hasNext()){
													...
													if (listaFieldPrimar.hasNext() && !strResulSetPks.trim().substring(strResulSetPks.trim().length()-1,strResulSetPks.trim().length()).equals(",") ){
														strResulSetPks=strResulSetPks+',';
													}
												}// while (listaFieldPrimar.hasNext()){
											}
											...
											while (itProperty.hasNext()){
												Property restoNulos= itProperty.next();
												strResulSetPks=strResulSetPks+", null";
											}
											strResulSetPks = strResulSetPks+")";
									...
									strResulSetPks=strResulSetPks+obtenerCadenaResultSet(pojo, subclass.getIdentifierProperty()); 
									/*
									String TypeSimp= warSupresor.getJavaTypeName(subclass.getIdentifierProperty(), true, false)  ;
									Iterator<Column> listColumnas = subclass.getIdentifierProperty().getColumnIterator() ;
									while (listColumnas.hasNext()){
										Column secuencia=listColumnas.next();
										strResulSetPks=strResulSetPks+"resultSet.get"+ BasicPOJOClass.beanCapitalize(WarningSupressorJdbc.getIntegerGetter(WarningSupressorJdbc.typeConverter(TypeSimp,false)))+"(\""+ ControllerUtils.findHibernateName(secuencia.getName()).toUpperCase()+"\")";
										System.out.println("Secuencia1:: " + secuencia.getName()+ ":" + TypeSimp + "-->" +WarningSupressorJdbc.typeConverter(TypeSimp,false));
									}
									*/
								} //end else if (c2j.isComponent(subclass.getIdentifier())){
								...
								*/
								...
								if (strResulSetPks.equals("") || strResulSetPks.substring(strResulSetPks.length()-1,strResulSetPks.length()).equals("(")){
									strResulSetPks=strResulSetPks+obtenerCadenaResultSet(pojo, prim); 
								}else{
									strResulSetPks=strResulSetPks+", " + obtenerCadenaResultSet(pojo, prim);  
								}								
								/*
						...
						while(colsPkCompuesta.hasNext()){
							...
							if (strResulSetPks.equals("")|| strResulSetPks.substring(strResulSetPks.length()-1,strResulSetPks.length()).equals("(")){
								strResulSetPks = strResulSetPks + obtenerCadenaResultSet(pojo, col); 
							}else{
								strResulSetPks = strResulSetPks + ", " + obtenerCadenaResultSet(pojo, col); 
							}
						}// while primarias.hasNext()){
					...
					//}else{//<#-- Propiedad que no es clave -->
					}	
				}
			}
			System.out.println(strResulSetPks);
			return strResulSetPks;
		}catch(Exception e){
			log.error("Error:" + e.getCause() + e.getMessage());
			return null;
		}
	...
	public List<String> selectFieldsMN(POJOClass pojo, Configuration cfg, Property propiedad){
		...
		try{
			...
			while (columnasHijos.hasNext()){
				...
				if (!c2h.isCollection(columnas)  && !c2h.isManyToOne(columnas) && !c2h.isOneToOne(columnas)){
		...
		try{
			...
			if(subclass.getIdentifierProperty().isComposite()){
				...
				while (primHijo.hasNext()){
					...
					recuperarHijo = recuperarHijo + "resultSet.get"+WarningSupressorJdbc.getIntegerGetter(WarningSupressorJdbc.typeConverter(warSupresor.getJavaTypeName(auxPrim, true, true),false))+"(\""+ auxPrim.getName().toUpperCase()+"\"), ";
				}
			}else{
				//subclase clave simple
				recuperarHijo = recuperarHijo + "resultSet.get"+WarningSupressorJdbc.getIntegerGetter(WarningSupressorJdbc.typeConverter(warSupresor.getJavaTypeName(subclass.getIdentifierProperty(), true, true),false))+"(\""+ subclass.getIdentifierProperty().getName().toUpperCase()+"\"), ";
			}
			...
			while (columnaHijos.hasNext()){
				...
				if (!c2h.isCollection(columnas)  && !c2h.isManyToOne(columnas) && !c2h.isOneToOne(columnas)){
					recuperarHijoaux = recuperarHijoaux + "resultSet.get"+WarningSupressorJdbc.getIntegerGetter(WarningSupressorJdbc.typeConverter(warSupresor.getJavaTypeName(columnas, true, true),false))+"(\""+ columnas.getName().toUpperCase()+"\"), ";
				}else{
